...
# --- 1. FUNZIONE DATABASE (CON RESET AUTOMATICO) ---
def init_db():
    conn = sqlite3.connect('beads.db', check_same_thread=False)
    c = conn.cursor()
    
    # TRUCCO PER RISOLVERE L'ERRORE: Eliminiamo la vecchia tabella se esiste
    # per ricostruirla con le nuove colonne (Prezzo, Designer, ecc.)
    c.execute("DROP TABLE IF EXISTS charms")
    
    # Creazione tabella aggiornata
    ...
    beads_master = [
        ...
        ('Trollbeads', 'TAGPE-00012', 'IMG_3861.jpeg', 'Canto della Balena', 'Whale\'s Song', "Voce misteriosa dell'oceano.", "Voice of the ocean.", 55.0, 'Lise Aagaard', 'Vetro / Argento', 0),
    ...
    # Inserimento dati (Abbiamo 11 colonne + il valore 'posseduto' fisso a 0)
    ...
    for i, row in df.iterrows():
        ...
        with col_img:
            ...
            if os.path.exists(row['img_filename']):
                st.image(row['img_filename'], width=80)
            else:
                st.write("🖼️")
