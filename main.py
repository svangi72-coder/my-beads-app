...
# La cartella fisica dove risiedono i file
IMG_FOLDER = os.path.join(BASE_DIR, 'immagini')

# Crea la cartella fisica se non esiste (necessario per il primo avvio)
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        # Percorsi aggiornati con prefisso immagini/ come richiesto
        beads_master = [
...
def mostra_beads(dataframe, is_collezione_personale=False):
    ...
    for i, row in dataframe.iterrows():
        with st.container():
            ...
            with col_img:
                # Il percorso completo viene costruito partendo dal valore nel database (es. immagini/fede.jpg)
                ...
                if full_img_path and os.path.exists(full_img_path):
                    ...
                    st.caption(f"Percorso non trovato: {img_relative_path}")
            with col_info:
...
if menu == "Catalogo Generale":
    ...
    with st.form("form_add", clear_on_submit=True):
        ...
        if st.form_submit_button("✨ Salva nel Catalogo"):
            # Percorso salvato nel database come immagini/nomefile.jpg
            ...
            conn.commit(); st.success(f"Salvato correttamente come {relative_name}")
    ...
    if not df_p.empty:
        ...
        st.bar_chart(df_p['materiale'].value_counts())import streamlit as st
import sqlite3
import pandas as pd
import os
from PIL import Image
